...
// MAGIC 
// MAGIC #Let’s start with a simple Pandas Series (as defined for x) and then apply the local function cubed() for the cubed calculation:
// MAGIC # Create a Pandas Series
// MAGIC x = pd.Series([1, 2, 3])
// MAGIC # The function for a pandas_udf executed with local Pandas data
// MAGIC print(cubed(x))
// MAGIC 
// MAGIC #Now let’s switch to a Spark DataFrame. We can execute this function as a Spark vectorized UDF as follows:
// MAGIC # Create a Spark DataFrame, 'spark' is an existing SparkSession
...
val airportsPath =
 "/FileStore/tables/airport_codes_na.txt"
